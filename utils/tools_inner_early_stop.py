...
class EarlyStopping:
    ...
    def __call__(self, vali_average_loss, args, model, path, variable):
        ...
        if args.tun_model:
            if score < self.best_avg_score[variable]:
                torch.save(model.state_dict(), f"{path}/{variable}_inner_paprams.pth")
                self.counter[variable] = 0
                self.best_avg_score[variable] = score
            else:
                self.counter[variable] += 1
