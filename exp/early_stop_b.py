...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        # 批量处理plugs
        batch_size = 4  # 每批处理的plug数量
        early_stop = [0] * self.args.plug_num
        count = [0] * self.args.plug_num
        
        for i in range(0, self.args.plug_num, batch_size):
            # 获取当前批次的plug范围
            batch_end = min(i + batch_size, self.args.plug_num)
            print(f"Processing plugs {i} to {batch_end-1}")
            
            # 串行处理当前批次的plugs
            for plug in range(i, batch_end):
                self.train_one_plug(
                    plug=plug,
