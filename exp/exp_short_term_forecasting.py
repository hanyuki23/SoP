...
import warnings
import numpy as np
from models.tuning import SimpleFCNet
import pandas
...
class Exp_Short_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        for i in range(self.args.pred_len):
            tun_models.append(SimpleFCNet(self.args, sequence_length=1, in_features=in_features))
    ...
    def _select_optimizer(self):
        ...
        tun_optim_models = []
        for i in range(self.args.pred_len):
            tun_optim_models.append(optim.AdamW(self.tun_models[i].parameters(), lr=self.args.learning_rate))
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.tun_model:
                    ...
                    outputs = torch.cat(output_finnal, dim=1)
            ...
            vali_loss = self.vali(train_loader, vali_loader, criterion, load_path)
            test_loss = vali_loss
            print("Epoch: {0}, Steps: {1} | Train Loss: {2:.7f} Vali Loss: {3:.7f} Test Loss: {4:.7f}".format(
                epoch + 1, train_steps, train_loss, vali_loss, test_loss))
            
            # tun model
            early_stopping(vali_loss, self.model, self.tun_models, self.args.tun_model, self.args.pred_len, path)
    ...
    def vali(self, train_loader, vali_loader, criterion, load_path):
    ...
    def test(self, setting, test=0):
        ...
        if 'Weekly_forecast.csv' in os.listdir(file_path) \
            ...
            f.write(setting + "  \n")
            f.write('smape:{}, mape:{}, mase:{}, owa:{}'.format(smape_results, mape, mase, owa_results))
            f.write('\n')
            f.write('\n')
            f.close()
        else:
