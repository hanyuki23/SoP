...
import warnings
import numpy as np
from models.tuning import SimpleFCNet_channel,SimpleFCNet2,SimpleFCNet_traffic,SimpleFCNet_ett,SimpleFCNet_exchange,SimpleFCNet,DLinear
...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.tun_model:
            for i in range(self.args.plug_num):
                tun_models.append(DLinear(self.args))                 
    ...
    def train(self, setting):
        ...
        self.args.cluster = train_cluster_h
        self.args.plug_num = len(train_cluster_h)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    x_out = torch.zeros([outputs.size(0),self.args.pred_len, self.args.c_out],dtype=outputs.dtype).to(outputs.device)
                    ...
                    if self.args.tun_model:
                        ...
                        if self.args.channel_fintune:
                            ...
                            for j in range(len(train_cluster_h)):
                                task_inds = train_cluster_h[j]
                                x_out[:,:,task_inds] = self.tun_models[j](outputs[:,:,task_inds] )
                        else:
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.tun_model:
                    if self.args.channel_fintune:
                        ...
                        for j in range(len(self.args.cluster)):
                            task_inds = self.args.cluster[j]
                            val_out = self.tun_models[j](outputs[:,:,task_inds])
                            true = batch_y[:,:,task_inds].detach().cpu()
                            pred = val_out.detach().cpu()
                            total_loss[j].append(criterion(pred, true))
                            x_out[:,:,task_inds] = val_out
                    else:
                        for j in range(self.args.pred_len // self.args.cseg_len):
                            output_finnal.append(self.tun_models[j](outputs[:, j*self.args.cseg_len:self.args.cseg_len*(j+1), :]))
    ...
    def test(self, setting, test=0):
        ...
        if test:
            ...
            if self.args.tun_model:
                ...
                if self.args.channel_fintune:
                    for i in range(len(self.args.cluster)):
